...
// Inicio del juego// Inicio del juego con animación de selección de imagen
startBtn.addEventListener("click", () => {
    juegoIniciado = false; // Bloqueamos mientras "elige"
    startBtn.disabled = true;

    let i = 0;
    const animacionInterval = setInterval(() => {
        const indice = Math.floor(Math.random() * imagenes.length);
        img.src = imagenes[indice];  // cambia imagen rápidamente
        i++;
        if (i >= 10) { // durará 10 cambios (aprox 1s si interval = 100ms)
            clearInterval(animacionInterval);

            // Imagen final del nivel
            const indiceFinal = Math.floor(Math.random() * imagenes.length);
            img.src = imagenes[indiceFinal];

            // Activamos el juego y timer
            juegoIniciado = true;
            segundos = 0;
            timerDisplay.textContent = formatearTiempo(segundos);
    ...
    }, 100); // cada 100ms cambia la imagen
});
