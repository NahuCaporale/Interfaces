...
function dibujarJuego() {
  ...
  for (let i = 0; i < 4; i++) {
    const fila = Math.floor(i / 2);//toma la parte entera , sies 0 fila 1 si es 1 fila 2
    const columna = i % 2;//calcula  la columna, si da 0 = columna 0, 1 columna 1
    const x = columna * TAMANIO_PIEZA; // calculamos las coordenadas en el canvas
...
function dibujarPieza(indice, x, y, filtro) {
  ...
  const fila = Math.floor(indice / 2);//calcula la fila de la pieza
  const columna = indice % 2;//calcula la columna de la pieza

  ctx.save();

  // mover x, y al centro de la pieza y no desde las esquinas
  ...
  ctx.drawImage(
    ...
    columna * (estadoJuego.imagenActual.width / 2),// sx: origen X en imagen
    fila * (estadoJuego.imagenActual.height / 2),// sy: origen Y en imagen
    estadoJuego.imagenActual.width / 2,// sWidth
    estadoJuego.imagenActual.height / 2,// sHeight
    -TAMANIO_PIEZA / 2,// dx: posición X en canvas
    -TAMANIO_PIEZA / 2, // dy: posición Y en canvas
    TAMANIO_PIEZA,
    TAMANIO_PIEZA
  );
...
if (canvas) {
  canvas.addEventListener("click", (e) => {
    ...
    manejarClickPieza(e, false);//click izquierdo
  });

  canvas.addEventListener("contextmenu", (e) => {
    e.preventDefault();
    if (!estadoJuego.estaJugando) return;
    manejarClickPieza(e, true);//click derecho
  });
}
...
function manejarClickPieza(e, esClickDerecho) {
  ...
  const x = (e.clientX - rect.left) * escalaX;//posicion en canvas
...
function rotarPieza(indice, sentidoHorario) {
  ...
  if (sentidoHorario) {
    pieza.rotacion = (pieza.rotacion + 90) % 360;//mantenmos la pieza entre 0 y 360
  } else {
...
function verificarVictoria() {
  const todasCorrectas = estadoJuego.piezas.every(
    (pieza) => pieza.rotacion === pieza.rotacionCorrecta//retorna true si cumplen todas
  );

  if (todasCorrectas) {//si todas en rotacion= 0, win
    detenerTemporizador();
